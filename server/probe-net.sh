...
for ct in $CTS; do
  ...
  # Разделитель: сосед по подсети и шлюз — разные пути на уровне кадров.
  # До соседа кадр идёт по его MAC, до Telegram — через MAC шлюза. Если
  # сосед отвечает, а Telegram нет, дело именно в записи о шлюзе, а не в
  # том, что контейнер вообще отрезан от моста. Входящие запросы от nginx
  # это не показывают: на них контейнер отвечает по MAC из полученного
  # кадра, шлюз для этого не нужен — потому основной сайт и отдаёт 200,
  # хотя наружу из него ничего не уходит.
  echo "  сосед по подсети (кадр по его MAC, без шлюза):"
  docker exec "$ct" sh -c "
    for peer in 172.18.0.4:80 172.18.0.5:8787; do
      ip=\${peer%%:*}; port=\${peer##*:}
      if command -v nc >/dev/null 2>&1 && nc -w 3 -z \$ip \$port 2>/dev/null; then
        echo \"    \$peer — отвечает\"
      else
        echo \"    \$peer — не отвечает\"
      fi
    done
  " 2>/dev/null || echo "    не выполнилось"
